...
packages, and own the files thereafter. The template `hyprland.conf` sources
`user.conf` (identity: personal binds, window rules) and `host.conf`
...
configurations, add `monitor=` lines to `~/.config/hypr/host.conf` in your
...
Bindings are defined in the template `hyprland.conf` (and thereafter in the
...
> `hyprland.conf` `input { kb_options=... }` block.
