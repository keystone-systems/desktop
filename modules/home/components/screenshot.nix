...
in
{
  config = mkIf (cfg.enable && cfg.environment == "hyprland") {
    # grim/slurp/satty/wayfreeze/hyprpicker/jq are installed OS-level by
    # ks.systems/desktop's hyprland NixOS module; the wrapper's store-path
    # references above keep its own closure complete regardless.
    home.packages = [
      keystoneScreenshot
    ];
  };
